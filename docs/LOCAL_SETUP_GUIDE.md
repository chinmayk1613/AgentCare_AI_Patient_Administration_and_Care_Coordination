...
### Test E - reminders and follow-up

1. Open **Reminders & follow-up** after completing a booking.
2. Verify the case number, appointment reminder, administrative follow-up task,
   scheduled IST times, and status come from persisted appointment data.
3. Reschedule the appointment and verify the old tasks are replaced with times
   calculated from the replacement slot.
4. Cancel an eligible appointment and verify its reminders become cancelled.
5. As authorized staff, record a synthetic clinician follow-up recommendation
   in the appointment detail. Confirm it appears separately from administrative
   reminders and is never generated by an agent.

### Test F - registration and hospital administration

1. As a patient, open **My profile**, update the synthetic phone, preferred
   language, or emergency contact, save, reload, and confirm persistence.
2. Confirm another patient cannot access that profile through the backend.
3. As staff, open **Hospital directory**, activate/deactivate a department or
   doctor, and verify the update is audit logged.
4. Create a future slot for an active doctor and verify it appears in
   transactional availability.
5. Confirm a duplicate doctor/time is rejected and a booked slot cannot be
   disabled or overwritten.

## 8A. Challenge evaluation coverage

The repository includes a line-by-line evidence matrix at
[`docs/challenge-compliance.md`](challenge-compliance.md). The required/core
criteria are implemented and wired:

- patient registration/profile update, intent detection, RAG routing, live
  availability, booking, rescheduling, cancellation, document coordination,
  confirmation, reminders, follow-up, and human escalation;
- patient and staff roles with backend ownership, permission, and reviewer
  checks;
- distinct Coordinator, Safety, Intent, Routing, Appointment, Document, and
  Follow-up responsibilities with persisted state hand-offs;
- Python/FastAPI, OpenAI integration, functional tools, persistent SQL,
  audit evidence, recovery/idempotency, environment configuration, synthetic
  data, tests, and a real user interface;
- staff directory and slot management with booked-slot protection.

External SMS/email delivery is deliberately represented as a persisted
notification/reminder task in this demonstration. A production notification
provider requires the hospital's approved endpoint, templates, consent rules,
and delivery credentials; no fake external delivery claim is made.
