...
python -m uvicorn app.main:app --reload --host 127.0.0.1 --port 8000
    ...
    story += [
        section_title("8A", "Reminders, profiles, administration, and coverage", s["h1"]),
        P("E. Reminders and follow-up", s["h2"]),
        B("Open Reminders & follow-up after booking; verify case number, task type, scheduled IST time, and persisted status.", s["bullet"]),
        B("Reschedule and verify reminders are rebuilt from the replacement slot; cancel and verify tasks stop.", s["bullet"]),
        B("Clinician recommendations remain clearly separate and are never generated by an agent.", s["bullet"]),
        P("F. Registration and hospital administration", s["h2"]),
        B("Patient: update synthetic phone, language, or emergency contact in My profile; reload to prove persistence and ownership.", s["bullet"]),
        B("Staff: activate/deactivate approved departments or doctors and create a future slot.", s["bullet"]),
        B("Verify duplicate doctor/time slots are rejected and booked slots are protected.", s["bullet"]),
        Spacer(1, 4 * mm),
        P("Challenge evaluation coverage", s["h2"]),
        wrapped_table(
            [
                ["Evaluation area", "Implemented evidence"],
                ["Core journey", "Profile, intent, RAG routing, live slots, book/reschedule/cancel, documents, confirmation, reminders, follow-up, escalation"],
                ["Roles", "Patient ownership + staff permissions; profile, directory, slot, reviewer, and audit APIs"],
                ["Agents/tools", "Distinct prompts and boundaries; OpenAI, RAG, MCP, SQL tools, durable checkpoints"],
                ["Safety", "No diagnosis/prescription/dosage; human review; private synthetic uploads; audit"],
                ["Engineering", "Python backend, D1/SQL, environment config, recovery/idempotency, tests, real UI"],
            ],
            [45 * mm, 121 * mm],
            font_size=7.0,
        ),
        P(
            "The demo persists reminder/notification tasks but does not falsely claim external SMS/email delivery. Production delivery requires an approved provider, consent rules, templates, credentials, retries, and delivery receipts.",
            s["danger"],
        ),
        P("Full line-by-line evidence: docs/challenge-compliance.md", s["callout"]),
        PageBreak(),
    ]
